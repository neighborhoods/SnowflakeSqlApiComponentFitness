...
class Proxy
{
    public function run(): void
    {
        ...
        $container = (new TinyContainerBuilder())
            ...
            ->addSourcePath('src/ClientV1/CancelRunningStatement')
        ...
        $initialResponse = $worker->callApi(
            $worker->buildEternalRunningRequest()
        );
        $worker->displayResult(
            $initialResponse
        );
        echo PHP_EOL . 'Cancelling the statement.' . PHP_EOL;
        $worker->displayResult(
            $worker->callApi(
                $worker->buildCancelationRequest($initialResponse->getStatementHandle())
            )
        );
    }
}
